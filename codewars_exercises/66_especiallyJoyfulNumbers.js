...
Complete the function which tests if a positive integer n is Harshad number, and returns True if the product 
of its digit sum and its digit sum reversed equals n; otherwise return False.
*/
...
function numberJoy(n) {
  let digitSum = String(n)
    .split("")
    .reduce((acc, currVal) => {
      return Number(currVal) + acc;
    }, 0);

  let digitsSumReverse = Number(String(digitSum).split("").reverse().join(""));
  return digitSum * digitsSumReverse === n ? true : false;
}
