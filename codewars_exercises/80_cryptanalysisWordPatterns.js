...
function wordPattern(word) {
  word = word.toLowerCase(); // lowercase the string
  const set = [...new Set(word)]; // create a set with unique values (removes duplicates)
  return word
    .split("")
    .map((letter) => set.indexOf(letter))
    .join("."); // split word into array, map through it, find the value of 'letter' in set, return it and join all values
}
