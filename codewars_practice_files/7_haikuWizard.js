...
A haiku is a traditional form of Japanese poetry, they normally have 3 lines with 5, 7 and 5 syllables respectively. 
...
Feeling a little concerned that I'm about to ask you to write poetry? Fear not! You only have to translate the haiku 
from numbers to words, no need to start looking frantically for creative inspiration. Let me explain:

I'm giving you:

a function, haikuWizard(), which you have to complete.
a big, predefined array with lots of words organised in subarrays! As you can see, each subarray has 10 members and 
they are all words with the same number of syllables. So words[0] contains words of 1 syllable, words[1] contains words 
...
Back to our function: haikuWizard() takes an array as its input. This array has 3 subarrays - each subarray represents 
a line of the haiku. For example:

[[52], [17, 23, 39, 18], [33, 22]]

For each number, the first digit tells you the number of syllables of the word, the second digit tells you which member 
of the relevant subarray it is.

For example:
52: This means that the first line is made up of one word with 5 syllables, at index 2 in words[4].
[17, 23, 39, 18]: This means that the second line is made up of 4 words. The first has 1 syllable and is at index 7 in 
words[0]. The second has 2 syllables and is at index3 in words[1]. And so on...

All you have to do is complete the function haikuWizard() so that it translates the given array back into the beautiful 
haiku it came from :)
...
- the first digit of each number in arr represents the number of syllables of the word, the second digit is its index in 
the relevant subarray of words.
