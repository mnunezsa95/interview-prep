...
"is2 sentence4 This1 a3". Given a shuffled sentence s containing no more than 9 words, reconstruct 
...
var sortSentence = function (s) {
  ...
  for (let i = 0; i < array.length; i++) {
    let currentElLength = [array[i].length - 1];
    indices[array[i][currentElLength]] = array[i];
  }
  for (let j = 0; j < array.length; j++) {
    resultString.push(indices[j + 1]);
    resultString[j] = resultString[j].slice(0, resultString[j].length - 1);
  }
