...
function parseHTMLColor(color) {
  ...
  function getRGBValue(v1, v2) {
    if (v1 in map) v1 = map[v1];
    if (v2 in map) v2 = map[v2];
    return v1 * 16 + +v2;
  }
  ...
  if (color.length == 3) {
    const redOne = color.slice(0, 1).split("");
    const greenOne = color.slice(1, 2).split("");
    const blueOne = color.slice(2).split("");

    rgb.r = getRGBValue(redOne, redOne);
    rgb.g = getRGBValue(greenOne, greenOne);
    rgb.b = getRGBValue(blueOne, blueOne);
  } else if (color.length == 6) {
    const [redOne, redTwo] = color.slice(0, 2).split("");
    const [greenOne, greenTwo] = color.slice(2, 4).split("");
    const [blueOne, blueTwo] = color.slice(4).split("");

    rgb.r = getRGBValue(redOne, redTwo);
    rgb.g = getRGBValue(greenOne, greenTwo);
    rgb.b = getRGBValue(blueOne, blueTwo);
  }
